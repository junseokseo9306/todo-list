            ...
            DragEvent.ACTION_DROP -> {
                ...
                when (viewId) {
                    cardItem, rvTodo, rvProgress, rvComplete -> {
                        ...
                        Log.d(
                            "AppTest",
                            "rvTodo id : ${rvTodo}, rvProgress id : ${rvProgress}, rvComplete id : ${rvComplete}"
                        )
                        Log.d("AppTest", "target id : ${target.id}")
                        if (viewSource != null) {
                            val source = viewSource.parent as RecyclerView // 선택한 아이템의 리사이클러뷰
                            val adapterSource =
                                source.adapter as TodoCardListAdapter // 위 리사이클러뷰의 어댑터를 가져옴 <- 이동하려는 것의 어댑터
                            val adapterTarget =
                                target.adapter as TodoCardListAdapter // 이동을 하고 놓을 곳의 어댑터
                            val positionSource =
                                source.getChildAdapterPosition(viewSource) // 이동하려는 아이템뷰의 현재 인덱스?
                            val sourceData =
                                adapterSource.currentList[positionSource] // 이동하려는 아이템뷰의 객체 데이터 <- Card
                            val listSource =
                                adapterSource.currentList.toMutableList() // submitList를 위함, listSource는 이동하려는 아이템뷰의 리사이클러뷰의 전체 리스트
                            ...
                            if (adapterSource == adapterTarget) {  // 같은 리사이클러뷰 내에서의 이동인 경우 <- 이 부분 없으면 이전처럼 복제 오류 발생
                                ...
                                val targetPosition =
                                    target.getChildAdapterPosition(view)  // 드래그 하고 카드 아이템뷰 위에 놓은 경우 여기로 온다,
                                ...
                                if (positionSource > targetPosition) {  // 아래에서 위로 이동 시, db 상에서 드롭 한 카드의 다음에 위치하도록 구현, 화면상에서는 드롭 한 카드의 위에 위치
                                    ...
                                    preOrder =
                                        if (targetPosition == adapterTarget.currentList.size - 1) {
                                            -1
                                        } else {
                                            adapterTarget.currentList[targetPosition + 1].order
                                        }
                                ...
                                if (positionSource != targetPosition) { // 드래그 후 자기 자신 위치에 놓으면 변화없음
                                    when (target.id) {
                                        rvTodo -> dataChangeListener.swapData(
                                            1,
                                            cardId,
                                            nextOrder,
                                            preOrder,
                                        ...
                                        rvProgress -> dataChangeListener.swapData(
                                            2,
                                            cardId,
                                            nextOrder,
                                            preOrder,
                                        ...
                                        rvComplete -> dataChangeListener.swapData(
                                            3,
                                            cardId,
                                            nextOrder,
                                            preOrder,
                            ...
                            if (positionTarget >= 0) {  // 다른 리사이클러뷰의 아이템뷰 위에 놓았을 때
                                ...
                                val preOrder =
                                    adapterTarget.currentList[targetPosition].order  // db 상에서 놓은 카드의 다음에 위치시키기 위함,  화면상에서는 놓은 카드의 위에 위치
                                ...
                                when (target.id) {
                                    rvTodo -> dataChangeListener.addDataAtInx(
                                        1,
                                        cardId,
                                        nextOrder,
                                        preOrder,
                                    ...
                                    rvProgress -> dataChangeListener.addDataAtInx(
                                        2,
                                        cardId,
                                        nextOrder,
                                        preOrder,
                                    ...
                                    rvComplete -> dataChangeListener.addDataAtInx(
                                        3,
                                        cardId,
                                        nextOrder,
                                        preOrder,
...
//                                val targetPosition =
//                                    target.getChildAdapterPosition(view)  // 이 경우 view가 리사이클러뷰 자체라 왼쪽같이 사용하면 오류 발생!!!
                                ...
                                if (adapterTarget.currentList.size == 0) { // 빈 섹션에 추가 시 pre, next order 모두 -1값을 할당한다
                                    ...
                                    preOrder = -1
                                    nextOrder =
                                        adapterTarget.currentList[adapterTarget.currentList.size - 1].order
                                }
...
//                                val nextSection = adapterTarget.currentList[targetPosition].section
                                ...
                                val nextSection =
                                    when (target.id) { // targetPosition을 사용할 수 없어서 놓는 영역의 리사이클러뷰에 따라 처리
                                        rvTodo -> "todo"
                                        rvProgress -> "doing"
                                        rvComplete -> "done"
                                        else -> ""
